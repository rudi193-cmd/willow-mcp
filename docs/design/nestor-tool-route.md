...
willow-mcp exposes ~100 verbs. An agent (or a human at a prompt) that knows
...
lives in a `[nestor]` optional extra in `pyproject.toml`; `pip install willow-mcp`
still stands up the server without it.
