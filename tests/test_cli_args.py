...
def _import_with(argv_tail, env=None):
    ...
    resolved values. Returns (port, host, base_url).

    SDK 2.x dropped `mcp.settings.host/port`: the listen address moved off the
    server object onto the transport (`run(transport=..., host=, port=)`), which
    is the stateless core making "where this instance listens" a property of the
    run rather than of the server. There is no longer a second copy to check
    against, so this asserts the module's resolved values only."""
...
def test_cli_flags_reach_fastmcp_and_base_url():
    port, host, base_url = _import_with(["--port", "8799", "--host", "0.0.0.0"])
    assert port == "8799"
    assert host == "0.0.0.0"
