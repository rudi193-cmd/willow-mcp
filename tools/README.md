...
| **`wtool.py`** | the substrate — call any of the 109 willow-mcp tools from a shell, so *any* script can do what a model does through the MCP client. | `$VP tools/wtool.py <tool> '<json>'` · `--list [substr]` |
...
The willow-mcp server exposes 109 tools. Most of the "jobs" a builder agent runs
