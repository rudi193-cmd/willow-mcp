...
| **`wtool.py`** | the substrate — call any of the 112 willow-mcp tools from a shell, so *any* script can do what a model does through the MCP client. | `$VP tools/wtool.py <tool> '<json>'` · `--list [substr]` |
...
The willow-mcp server exposes 112 tools. Most of the "jobs" a builder agent runs
